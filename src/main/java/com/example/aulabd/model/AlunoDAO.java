...
public class AlunoDAO {
    ...
    public Aluno buscarPorIdExato(String id) {
        List<Aluno> result = jdbc.query(
            "SELECT id, nome, cpf, foto FROM aluno WHERE id = ?::uuid", alunoMapper, id);
        return result.isEmpty() ? null : result.get(0);
    }

    public List<Aluno> listarTodos() {
        return jdbc.query("SELECT id, nome, cpf, foto FROM aluno ORDER BY nome", alunoMapper);
    }

    public List<Aluno> buscarPorNome(String nome) {
        return jdbc.query(
            "SELECT id, nome, cpf, foto FROM aluno WHERE LOWER(nome) LIKE LOWER(?) ORDER BY nome",
            alunoMapper, "%" + nome + "%");
    }
    ...
    public List<Aluno> buscarPorCpf(String cpf) {
        ...
        return jdbc.query(
            "SELECT id, nome, cpf, foto FROM aluno WHERE cpf LIKE ? ORDER BY nome",
            alunoMapper, "%" + limpo + "%");
    }

    public List<Aluno> buscarPorId(String id) {
        return jdbc.query(
            "SELECT id, nome, cpf, foto FROM aluno WHERE CAST(id AS TEXT) LIKE ? ORDER BY nome",
            alunoMapper, "%" + id + "%");
    }
